"""Authenticate against bookings@glnkco.com's NextAuth credentials endpoint and verify
the user is the system admin.

Reuses cal_client.cal_web.login() to do the actual login (CSRF + credentials
callback). The role check then GETs bookings@glnkco.com's /api/auth/session through the
...
# INACTIVE_ADMIN means the role is downgraded inside bookings@glnkco.com (e.g. because
# the admin hasn't enabled 2FA yet) — they're still the system administrator,
# we just can't depend on every bookings@glnkco.com admin-feature being usable. For the
...
def authenticate_admin(cal_web_base: str, *, email: str, password: str) -> CalAdminUser | None:
    """Verify bookings@glnkco.com credentials AND that the user has an admin role.
    ...
    try:
        ...
        log.info("admin login: bookings@glnkco.com login rejected for %s (%s)", email, exc)
        return None
